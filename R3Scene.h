...
#define N_CHARACTERS 10       // number of starting enemies
#define MAX_ENEMIES 35        // maximum number of enemies
#define TERRAIN_X_SIZE 300    // terrain grid size x-direction
#define TERRAIN_Y_SIZE 300    // terrain grid size y-direction
#define TERRAIN_DENSITY .5    // distance between grid points
#define TERRAIN_MAX_HEIGHT 7  // maximum height of the terrain
...
struct R3Scene {
 public:
  ...
  void refreshTerrainNormal(int x, int y, int z);
  double getRealZValue(R3Point point);
  double getRealZValue(R3Vector vector);
  double getRealZValue(double X, double Y);
  R3Intersection IntersectTerrain(R3Point start, R3Point end);
  R3Intersection IntersectTriangle(R3Point start, R3Point end, int x, int y, int z);
  bool LocationValid(double x, double y);
